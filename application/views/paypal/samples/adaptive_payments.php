...
<p><pre><code>
    function Convert_currency_sample()
    {
        ...
        $PayPalResult = $this->Adaptive_payments_model->Convert_currency($PayPalRequestData);

        if(!$PayPalResult)
        {
            $errors = $this->Adaptive_payments_model->errors;
            $this->load->view('paypal/samples/paypal_error',$errors);
        }
        else
        {
            // Successful call.  Load view or whatever you need to do here.
            $this->load->view('paypal/samples/paypal_result',$PayPalResult);
        }
    }
    </code></pre>
</p>
<p><a href="<?php echo site_url('paypal/samples/adaptive_payments_sample/convert_currency_sample'); ?>" target="_blank">Run the code above and see the output.</a></p>
<h2>Pay (Chained)</h2>
...
<p><pre><code>
    function Pay_chained_sample()
    {
        // Set InvoiceID
        $demo_invoice_id = 'DEMOINV-'.rand();       // Random InvoiceID for demo purpose

        // Prepare request arrays
        $PayRequestFields = array(
            'ActionType' => 'PAY', 								                // Required.  Whether the request pays the receiver or whether the request is set up to create a payment request, but not fulfill the payment until the ExecutePayment is called.  Values are:  PAY, CREATE, PAY_PRIMARY
            'CancelURL' => site_url('paypal/samples/adaptive_payments_sample/pay_chained_cancelled_sample'), 	// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
            ...
            'ReturnURL' => site_url('paypal/samples/adaptive_payments_sample/pay_chained_completed_sample'),     // Required.  The URL to which the sender's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
            'ReverseAllParallelPaymentsOnError' => '', 			                // Whether to reverse parallel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
        ...
        $Receiver = array(
            ...
            'InvoiceID' => $demo_invoice_id, 											            // The invoice number for the payment.  127 char max.
        ...
        $PayPalResult = $this->Adaptive_payments_model->Pay($PayPalRequestData);
        ...
        {
            // Successful call.
            // In most cases you would automatically redirect to the returned 'RedirectURL' by using: redirect($PayPalResult['PayPalResult']['RedirectURL'],'Location');

            // PayPal result and 'continue' link are displayed here for demo purposes
            print_r($PayPalResult);
            echo '< a href="'.$PayPalResult['PayPalResult']['RedirectURL'].'">CLICK TO CONTINUE TO PAYMENT< /a>';
            exit;
        }
    }

    function Pay_chained_completed_sample()
    {
        echo 'Pay - SUCCESS!';
        exit;
    }

    function pay_chained_cancelled_sample()
    {
        echo 'Pay - CANCELLED!';
        exit;
    }
...
<p><a href="<?php echo site_url('paypal/samples/adaptive_payments_sample/pay_chained_sample'); ?>" target="_blank">Run the code above and see the output.</a></p>
<p>&nbsp;</p>
</body>
</html>